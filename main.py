...
@router.post("/download/file", summary="Download file (下载文件)")
...
@router.get("/download/list", summary="Download file list (文件下载列表)")
async def download_list_with_aria2c(hh: dict = Depends(auth)):
    result = await views.get_download_list(hh)
    return result


@router.post("/download/status/update", summary="update download task status (更新下载任务的状态)")
